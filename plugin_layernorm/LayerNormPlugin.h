...
public:
    LayerNormPlugin(const std::string& name, float bias) : name_(name), bias_(bias)
    {
        WHERE_AM_I();
    }
    ...
    {
        WHERE_AM_I();
        memcpy(&bias_, data, sizeof(bias_));
    }
    ...
    {
        WHERE_AM_I();
        memcpy(buffer, &bias_, sizeof(bias_));
    }
  
    IPluginV2DynamicExt* clone() const noexcept override
    {
        WHERE_AM_I();
        return new LayerNormPlugin(name_, bias_);
    }
...
public:
    ...
    IPluginV2* createPlugin(const char* name, const PluginFieldCollection* fc) noexcept override
    {
        ...
        for (int i = 0; i < fc->nbFields; i++)
        {
            ...
            {
                bias = *static_cast<const float *>(fc->fields[i].data);
            }
        }
        return new LayerNormPlugin(name, bias);
    }
